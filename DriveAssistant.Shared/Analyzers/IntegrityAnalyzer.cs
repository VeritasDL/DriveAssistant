...
namespace FATX.Analyzers
{
    public class IntegrityAnalyzer
    {
        ...
        /// </summary>
        private Dictionary<uint, List<DatabaseFile>> clusterMap = new Dictionary<uint, List<DatabaseFile>>();
        ...
        {
            var dirent = databaseFile.GetDirent();
            if (databaseFile.HasManualRanking())
            {
                databaseFile.SetRankingReason("Manual status override");
                return;
            }
            ...
            if (!databaseFile.IsDeleted)
            {
                if (!dirent.IsDeleted())
                {
                    ...
                    databaseFile.SetRankingReason("Active file system entry (not deleted)");
                }
            }
            else
            {
                ...
                if (collisions.Count == 0)
                {
                    ...
                    databaseFile.SetRankingReason("Recovered deleted entry with no cluster collisions");
                }
                else
                {
                    ...
                    if (WasModifiedLast(databaseFile, collisions))
                    {
                        ...
                        databaseFile.SetRankingReason("Recovered entry with collisions, but appears most recently modified");
                    }
                    else
                    {
                        ...
                        if (collisions.Count != numClusters)
                        {
                            ...
                            databaseFile.SetRankingReason("Recovered entry with partial cluster overwrite/collision");
                        }
                        else
                        {
                            // Every cluster appears to have been overwritten
                            databaseFile.SetRanking((4));
                            databaseFile.SetRankingReason("Recovered entry appears fully overwritten by newer data");
                        }
                    }
                }
            }
        ...
        {
            return clusterMap.TryGetValue(cluster, out var occupants) ? occupants : null;
        }
    }
}
