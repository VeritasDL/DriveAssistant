...
| PlayStation Portable / PS Vita | PSP dev/test UMD and Memory Stick workflows, PS Vita package/devkit media | PSP UMD ISO/CSO images mount through ISO9660 for browsing/export, PSP EBOOT/PBP sections are exposed, PSP Memory Stick FAT images mount through generic FAT support, and PS Vita VPK/ZIP packages expose entries such as `sce_sys/param.sfo`. Encrypted Vita PFS/NAND/CMA content requires matching keys or prior decryption. |
